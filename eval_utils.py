...
def eval_split(model, loader, eval_kwargs={}):
    ...
    while True:
        ...
        sg_data = {
            key: data['sg_data'][key] if data['sg_data'][key] is None else torch.from_numpy(
                data['sg_data'][key]).cuda()
            for key in data['sg_data']}

        # forward the model to also get generated samples for each image
        with torch.no_grad():
            seq = model(sg_data, fc_feats, att_feats, relas, att_masks, opt=eval_kwargs, mode='sample')[
                0].data
        # Print beam search
