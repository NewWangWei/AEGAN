...
def train(opt):
    ...
    if opt.reduce_on_plateau:
        ...
        optimizer = utils.ReduceLROnPlateau(optimizer, factor=0.8, patience=3)
    else:
        optimizer = utils.build_optimizer(decoder.parameters(), opt)
    # optimizer = utils.build_optimizer(decoder.parameters(), opt)
    ...
    while True:
        ...
        tmp = [data['fc_feats'], data['att_feats'], data['labels'], data['tags'], data['masks'], data['att_masks'],
               data['node_labels'], data['node_masks'],
               data['verbs']]
        tmp = [_ if _ is None else torch.from_numpy(_).cuda() for _ in tmp]
        fc_feats, att_feats, labels, tags, masks, att_masks, node_labels, node_masks, weak_relas = tmp
        ...
        optimizer.zero_grad()
        if not sc_flag:
            out = decoder(sg_data, fc_feats, att_feats, labels, weak_relas, att_masks)
            loss_words = crit(out[0], labels[:, 1:], masks[:, 1:])
            loss = loss_words
            if opt.gamma1:
                loss_tags = crit(out[1], tags[:, 1:], masks[:, 1:])
                loss += opt.gamma1 * loss_tags
            if opt.gamma2:
                loss_node = crit(out[2], node_labels, node_masks)
                loss += opt.gamma2 * loss_node

        else:
            gen_result, sample_logprobs, core_args = decoder(sg_data, fc_feats, att_feats, weak_relas, att_masks,
                                                             opt={'sample_max': 0, 'return_core_args': True},
                                                             mode='sample')
            reward = get_self_critical_reward(decoder, core_args, sg_data, fc_feats, att_feats, weak_relas, att_masks,
                                              data, gen_result, opt)
            loss = rl_crit(sample_logprobs, gen_result.data, torch.from_numpy(reward).float().cuda())

        accumulate = accumulate + 1
        loss = loss.mean() / opt.accumulate_number
        loss.backward()

        # 3. Update model
        if accumulate % opt.accumulate_number == 0:
            accumulate = 0
