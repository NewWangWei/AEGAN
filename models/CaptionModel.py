...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, core_args, **kwargs):
        ...
        core_args = [_.chunk(group_size) if (_ is not None and type(_) is not list) else [_] * group_size for _ in
                     core_args]
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    logprobs_table[divm], _, state_table[divm] = self.get_logprobs_state(it.cuda(), state_table[divm],
                                                                                         core_args[divm])
